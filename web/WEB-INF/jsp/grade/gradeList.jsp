        ...
        $(function() {
            ...
            $('#dataList').datagrid({
                ...
                columns: [[
                    ...
                    {field:'gid',title:'年级编号',width:80, sortable: true},
                    {field:'gradeName',title:'年级名称',width:200},
                    {field:'courses',title:'年级课程',width:650,
                        //value 就是当前的值，即 courses 所对应的数组
                        //row 当前行所对应的 json
                        //index 当前渲染到第几行了
            ...
            $("#delete").click(function(){
                //先获取选中的行
                var selectRow = $("#dataList").datagrid("getSelected");//{gid:xxx,gradeName:xxx,courses:[{},{}]}
                if(selectRow == null){
                    $.messager.alert("消息提醒", "请选择数据进行删除!", "warning");
                } else{
                    var gradeid = selectRow.gid;
                    $.messager.confirm("消息提醒", "将删除与【"+selectRow.gradeName+"】年级相关的所有数据(包括班级,学生)，确认继续？", function(r){
                        if(r){
                            $.ajax({
                                type: "delete",
                                url: "/s/grade?gid=" + gradeid,
...
<div id="addDialog" style="padding: 10px">
    <form id="addForm" method="post">
        <table cellpadding="8" >
            ...
            <tr>
                <td>选择课程:</td>
                <td><select id="add_courseList" style="width: 200px; height: 30px;" name="cid" data-options="required:true, missingMessage:'请选择课程'" ></select></td>
            </tr>
        </table>
    </form>
</div>
